...
{
    public class HeadSelectorUI : MonoBehaviour
    {
        ...
        List<HeadTypeDef> all;

        void Start()
        {
            all = new List<HeadTypeDef>(DefDatabase<HeadTypeDef>.All());
            Populate(all);
        ...
        void Populate(List<HeadTypeDef> defs)
        {
            ...
            foreach (var def in defs)
            {
                ...
                item.Setup(def.displayName, def.sprites.north, def.sprites.east, def.sprites.south,
                    () => { drawer.SetHead(def); CharacterProfile.head = def; });
            }
        }
    }
}
